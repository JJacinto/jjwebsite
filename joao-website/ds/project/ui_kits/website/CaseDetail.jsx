// JJ Design System — Case Study Detail View
// Exports: JJCaseDetail
...
function JJCaseDetail({ caseId, setPage }) {
  ...
  const bodyText = { fontFamily:T.fontBody, fontSize:'0.95rem', color:T.fg2, lineHeight:1.8, marginBottom:16 };

  return (
    <div style={{ background:T.bg, paddingTop:'clamp(80px,12vw,120px)', paddingBottom:T.sectionGap }}>
      <div style={containerStyle}>

        {/* Back */}
        <div onClick={() => setPage('cases')} style={{ display:'inline-flex', alignItems:'center', gap:8, fontFamily:T.fontBody, fontSize:'0.9rem', fontWeight:400, color:T.fg2, cursor:'pointer', marginBottom:40, transition:'color 0.2s' }}
        ...
        {/* Hero */}
        <div style={{ paddingBottom:48, borderBottom:`1px solid ${T.border}`, marginBottom:48 }}>
          <div style={{ fontFamily:T.fontBody, fontSize:'0.75rem', color:T.fg3, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:16 }}>{c.cat}</div>
          <h1 style={{ fontFamily:T.fontDisplay, fontWeight:900, fontSize:'clamp(2rem,4vw,3rem)', letterSpacing:'-0.035em', lineHeight:1.15, marginBottom:20, color:T.fg1 }}>{c.title}</h1>
          <p style={{ ...bodyText, fontSize:'1.05rem', maxWidth:680, marginBottom:36 }}>{c.subtitle}</p>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:24, marginBottom:36 }}>
            {[['Company',c.company],['Role',c.role],['Year',c.year],['Duration',c.duration]].map(([label,val],i)=>(
              <div key={i}>
                <div style={{ fontFamily:T.fontBody, fontSize:'0.72rem', color:T.fg3, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:6 }}>{label}</div>
                <div style={{ fontFamily:T.fontBody, fontSize:'0.92rem', color:T.fg1 }}>{val}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {c.keywords.map((k,i)=>(
              <span key={i} style={{ padding:'6px 14px', border:`2px solid rgba(28,26,23,0.10)`, borderRadius:'100px', fontFamily:T.fontBody, fontSize:'0.8rem', color:T.fg2, background:T.surface }}>
                {k}
              </span>
            ))}
          </div>
        ...
        {/* Hero image */}
        <img src={c.img} alt={c.title} style={{ width:'100%', borderRadius:16, marginBottom:60, objectFit:'cover', maxHeight:440, border:`2px solid rgba(28,26,23,0.08)` }} />

        {/* Achievements */}
        <div style={{ background:T.accentSubtle, borderRadius:16, padding:32, marginBottom:60 }}>
          <h3 style={{ fontFamily:T.fontDisplay, fontWeight:900, fontSize:'1rem', letterSpacing:'-0.02em', marginBottom:14, color:T.fg1 }}>Key outcomes</h3>
          <ul style={{ listStyle:'none', padding:0 }}>
            {c.achievements.map((a,i)=>(
              <li key={i} style={{ fontFamily:T.fontBody, fontSize:'0.9rem', color:T.fg2, lineHeight:1.8, paddingLeft:20, position:'relative', marginBottom:4 }}>
                <span style={{ position:'absolute', left:4, color:T.accent, fontWeight:700 }}>→</span>{a}
              </li>
            ))}
          </ul>
        </div>
        ...
        {c.sections.map((s,i)=>(
          <div key={i} style={{ marginBottom:48 }}>
            <h2 style={{ fontFamily:T.fontDisplay, fontWeight:900, fontSize:'clamp(1.5rem,2.5vw,2rem)', letterSpacing:'-0.03em', marginBottom:20, color:T.fg1 }}>{s.h}</h2>
            <p style={bodyText}>{s.body}</p>
          </div>
        ))}

        {/* Pull quote */}
        <blockquote style={{ borderLeft:`3px solid ${T.accent}`, paddingLeft:24, margin:'48px 0', fontFamily:T.fontBody, fontSize:'1.05rem', fontStyle:'italic', color:T.fg1, lineHeight:1.7 }}>
          "{c.quote}"
        </blockquote>

        {/* Outcome stats */}
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${c.outcomes.length},1fr)`, gap:32, marginTop:60, paddingTop:48, borderTop:`1px solid ${T.border}` }}>
          {c.outcomes.map((o,i)=>(
            <div key={i} style={{ textAlign:'center' }}>
              <div style={{ fontFamily:T.fontDisplay, fontWeight:900, fontSize:'clamp(2rem,4vw,3rem)', color:T.accent, letterSpacing:'-0.04em', marginBottom:8 }}>{o.n}</div>
              <div style={{ fontFamily:T.fontDisplay, fontWeight:900, fontSize:'0.95rem', letterSpacing:'-0.01em', marginBottom:6, color:T.fg1 }}>{o.title}</div>
              <div style={{ fontFamily:T.fontBody, fontSize:'0.8rem', color:T.fg3, lineHeight:1.5 }}>{o.desc}</div>
            </div>
          ))}
        </div>
